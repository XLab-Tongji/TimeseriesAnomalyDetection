...
class DataLoader():
    def __init__(self):
        ...
        self.padding = 12                # the gap between the windows
        self.num_classes = 2
        self.data_window = 12       # the length of every window
    ...
    def load_aiops_data(self):
        ...
        self.dataset=[]
        self.originalAnoma=[]
        for m in range(len(self.data["value"])):
            self.dataset.append([m,self.data["value"][m]])
            if self.data["label"][m]==1:
                self.originalAnoma.append(m)
    ...
    def get_test_data(self):
        ...
        return self.x_test, self.y_test,self.dataset,self.originalAnoma
