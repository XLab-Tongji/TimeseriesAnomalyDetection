# -*- coding: utf-8 -*-
"""
Created on Fri Dec 25 19:43:25 2020
...
import torch
import pickle
from TimeseriesAnomalyDetection.RNN import preprocess_data
from TimeseriesAnomalyDetection.RNN.model import model
...
from TimeseriesAnomalyDetection.RNN.anomalyDetector import fit_norm_distribution_param
from TimeseriesAnomalyDetection.RNN.anomalyDetector import anomalyScore
from TimeseriesAnomalyDetection.RNN.anomalyDetector import get_precision_recall


class Arg():
    def __init__(self,prediction_window_size,data,filename,model,save_fig,compensate,beta):
        ...
        self.beta=beta
data1='power_demand'
filename1='power_data'
model1='LSTM'
args_ = Arg(10,data1,filename1,model1,False,False,1.0)
print('-' * 89)
print("=> loading checkpoint ")
checkpoint = torch.load(str(Path('save',args_.data,args_.model,'checkpoint',args_.filename).with_suffix('.pth')))
...
try:
    ...
    for channel_idx in range(nfeatures):
        ...
        if args.save_fig:
            save_dir = Path('result',args.data,args_.filename,args.model).with_suffix('').joinpath('fig_detection')
...
save_dir = Path('result', args.data, args_.filename,args.model).with_suffix('')
save_dir.mkdir(parents=True, exist_ok=True)
