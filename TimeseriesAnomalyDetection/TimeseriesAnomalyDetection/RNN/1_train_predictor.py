...
args = parser.parse_args() #解析输入参数
# Set the random seed manually for reproducibility.
# 在神经网络中，参数默认是进行随机初始化的。这样设置使得每次训练时的初始化固定
...
def train(args, model, train_dataset,epoch):
    ...
    with torch.enable_grad():
        ...
        for batch, i in enumerate(range(0, train_dataset.size(0) - 1, args.bptt)):
            ...
            outVals=[]
            hids1 = []
            #上一个state的输出作为下一个state的输入
            #使用最开始的一个batch数据预测接下来50个batch的数据值，比较得损失值
