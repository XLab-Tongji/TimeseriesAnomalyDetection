...
import torch
import pickle
import TimeseriesAnomalyDetection.RNN.preprocess_data
...
parser.add_argument('--model', type=str, default='LSTM',
                    help='model type:now only LSTM and GRU')
...
checkpoint = torch.load(str(Path('./TimeseriesAnomalyDetection/RNN/save',args_.data,args_.model,'checkpoint',args_.filename).with_suffix('.pth')))
...
try:
    ...
    for channel_idx in range(nfeatures):
        ...
        if args.save_fig:
            save_dir = Path('.\\TimeseriesAnomalyDetection\\RNN\\result',args.data,args_.filename,args.model).with_suffix('').joinpath('fig_detection')
...
save_dir = Path('.\\TimeseriesAnomalyDetection\\RNN\\result', args.data, args_.filename,args.model).with_suffix('')
save_dir.mkdir(parents=True, exist_ok=True)
