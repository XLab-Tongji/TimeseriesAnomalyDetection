...
class PickleDataLoad(object):
    def __init__(self, data_type, filename, augment_test_data=True):
        ...
        self.trainData, self.trainLabel = self.preprocessing(Path('.\\TimeseriesAnomalyDetection\\RNN\\dataset',data_type,'labeled','train',filename),train=True)
        self.testData, self.testLabel = self.preprocessing(Path('.\\TimeseriesAnomalyDetection\\RNN\\dataset',data_type,'labeled','test',filename),train=False)
